...
$$ Retention\ Rate = \frac{\text{number of people in the same cohort who revisited during the period}}{\text{number of people in the same cohort at the start of the period}} \times 100 $$
...
I have added additional variables such as campaign_type, country, and payer_flag. These will increase the complexity in the following sections. However, because in Section 1, the queries only needs to be run once and the results are saved to a middle table, I can include as many potentially useful categorical variables as needed in Section 1, and choose only the relevant ones in Section 2, or play around different variables and check the outcomes.
...
## Conclusion
I hope these points help you analyze retention rates more effectively and with greater confidence. Retention analysis can initially feel complex, but breaking it down into clear steps, visualizations, and structured SQL logic makes the process much more manageable. I strongly encourage testing these methods with your own data, as real‑world use cases often reveal insights that theoretical examples cannot. As you gain experience, you will also become more comfortable interpreting retention charts and explaining them to stakeholders. Ultimately, mastering retention analysis provides a strong foundation for understanding user behavior and making data‑driven business decisions.
